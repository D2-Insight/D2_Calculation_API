...
pub enum DpsSettings {
    ...
    InferReloadOverride(bool), //infer the reload override from the loadout|                    default: false
}
    ...
    pub fn is_met(&self, state: DpsSimulationInstance) -> bool {
        match self {
            ConditionSupplier::AbilityAvailable(ability) => match ability {
                AbilityTypes::Melee => {
                    if state.dps_data.melee.is_some() {
                        state.dps_data.melee.unwrap().available
                    } else {
                        false
                    }
                }
                ...
                AbilityTypes::Grenade => {
                    if state.dps_data.grenade.is_some() {
                        state.dps_data.grenade.unwrap().available
                    } else {
                        false
                    }
                }
                AbilityTypes::Super => {
                    if state.dps_data._super.is_some() {
                        state.dps_data._super.unwrap().available
                    } else {
                        false
                    }
                }
                AbilityTypes::Class => {
                    if state.dps_data.class.is_some() {
                        state.dps_data.class.unwrap().available
                    } else {
                        false
                    }
                }
            ...
            ConditionSupplier::WeaponEmpty(slot) => match slot {
                WeaponSlot::Kinetic => {
                    if state.dps_data.kinetic.is_some() {
                        state.dps_data.kinetic.unwrap().magazine > 0.0
                    } else {
                        false
                    }
                }
                WeaponSlot::Energy => {
                    if state.dps_data.energy.is_some() {
                        state.dps_data.energy.unwrap().magazine > 0.0
                    } else {
                        false
                    }
                }
                WeaponSlot::Power => {
                    if state.dps_data.power.is_some() {
                        state.dps_data.power.unwrap().magazine > 0.0
                    } else {
                        false
                    }
                }
            ...
            ConditionSupplier::WeaponBelow(slot, percent) => match slot {
                WeaponSlot::Kinetic => {
                    if state.dps_data.kinetic.is_some() {
                        let data = state.dps_data.kinetic.unwrap();
                ...
                WeaponSlot::Energy => {
                    if state.dps_data.energy.is_some() {
                        let data = state.dps_data.energy.unwrap();
                        data.magazine / data.base_magazine < *percent
                    } else {
                        false
                    }
                ...
                WeaponSlot::Power => {
                    if state.dps_data.power.is_some() {
                        let data = state.dps_data.power.unwrap();
                        data.magazine / data.base_magazine < *percent
                    } else {
                        false
                    }
            ...
            ConditionSupplier::IsStowed(slot) => match slot {
                WeaponSlot::Kinetic => state.active_weapon == 1,
                WeaponSlot::Energy => state.active_weapon == 2,
                WeaponSlot::Power => state.active_weapon == 3,
            },
